...
/// Filter removes all unwanted sections from document. The implementation
/// simply looks for "remove" metadata in documents and removes all that blocks of
/// documentation.
class RemoveUnwantedSectionsFilter: DocumentFilter {
    
    func setUpFilter(dataProvider: DocumentFilterDataProvider) -> Bool {
        Console.info("Removing unwanted sections...")
        return true
    }
